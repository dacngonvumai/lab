...
@Component({
  selector: 'app-bycate',
  templateUrl: './bycate.component.html',
  styleUrls: ['./bycate.component.css']
})
export class BycateComponent implements OnInit {
  ...
  ngOnInit() {
    this.route.paramMap.subscribe(para => {
      this.aId = para.get('id');
    })
    this.dulieu.forEach(dl => {
      if(dl.cid == this.aId) {
        this.cate.push(dl);
      }
    })
